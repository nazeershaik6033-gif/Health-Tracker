...
export interface PortionSheetProps {
  ...
  onClose: () => void;
  onConfirm: (qty: number, servingLabel: string) => void;
...
export function PortionSheet({
  ...
  const [servingLabel, setServingLabel] = useState(initialServingLabel ?? fallback.label);
    ...
    <BottomSheet
      ...
      footer={
        <div className="flex gap-2">
          {onDelete && (
            <Button variant="secondary" onClick={onDelete} aria-label={`Remove ${title}`}>
          ...
          <Button
            ...
            full
            disabled={quantity <= 0}
            onClick={() => onConfirm(quantity, servingLabel)}
          >
        ...
        <div className="flex items-end gap-2">
          ...
          <div className="mb-1 flex gap-1">
            <Button
              ...
              onClick={() => setQty(String(Math.max(0, Math.round((quantity - 0.5) * 2) / 2)))}
            >
              −
            </Button>
            <Button
              ...
              onClick={() => setQty(String(Math.round((quantity + 0.5) * 2) / 2))}
            >
              +
            </Button>
          </div>
